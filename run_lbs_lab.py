...
def prepare_posedirs(posedirs: torch.Tensor, expected_pose_dim: int):
    """
    Different SMPL files may store posedirs as [P, V*3] or its transpose.
    The official lbs() implementation expects [P, V*3].
    """
    if posedirs.dim() != 2:
    ...
    raise RuntimeError(
        f"posedirs shape does not match pose_feature: posedirs.shape={tuple(posedirs.shape)}, "
...
def main(args):
    ...
    expected_model = os.path.join(model_dir, "smpl", "SMPL_NEUTRAL.pkl")
    if not os.path.exists(expected_model):
        raise FileNotFoundError(
            f"SMPL model file not found: {expected_model}\n"
            "Please place the official model at models/smpl/SMPL_NEUTRAL.pkl."
        )
    ...
    if joint_id < 0 or joint_id >= model.lbs_weights.shape[1]:
        raise ValueError(
            f"joint_id out of range: {joint_id}; valid range is [0, {model.lbs_weights.shape[1] - 1}]"
        )
    ...
    print(f"manual vs official mean absolute error: {mean_err:.10f}")
    print(f"manual vs official max absolute error: {max_err:.10f}")
    print(f"results saved to: {out_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-dir", type=str, default="./models", help="model directory containing smpl/SMPL_NEUTRAL.pkl")
    parser.add_argument("--out-dir", type=str, default="./outputs", help="output directory")
    parser.add_argument("--joint-id", type=int, default=18, help="joint id whose weights will be visualized")
    parser.add_argument("--num-betas", type=int, default=10, help="number of shape parameters")
